...
public class IngredientAliasService implements IngredientAliasUseCase {
        ...
        public UpdateAliasAllResDto updateAliasAll(UpdateAliasAllReqDto req) {
                ...
                Map<String, IngredientAliasEntity> existingAliasMap = existingAliases.stream()
                ...
                List<IngredientAliasEntity> toDelete = existingAliases.stream()
                                .filter(entity -> !newAliasSet.contains(entity.getAlias()))
                                .collect(Collectors.toList());

                // 추가할 별칭: 새 목록에는 있지만 기존에는 없는 것
                List<IngredientAliasEntity> toAdd = newAliasSet.stream()
                ...
                // 삭제 실행
                if (!toDelete.isEmpty()) {
                        ingredientAliasJpaRepository.deleteAll(toDelete);
                }

                // 추가 실행
                if (!toAdd.isEmpty()) {
                        ingredientAliasJpaRepository.saveAll(toAdd);
                }
        ...
        public UpdateAliasResDto updateAlias(UpdateAliasReqDto req) {
                ...
                ingredientAliasJpaRepository
                                .findByIngredientIdAndLocale(aliasEntity.getIngredientId(), aliasEntity.getLocale())
                                .stream()
