...
public class RecipeService implements RecipeUseCase {
        ...
        public RecommendRecipeResDto recommendRecipes(RecommendRecipeReqDto req) {
                ...
                if (!allMissingIngredientIds.isEmpty()) {
                        List<IngredientEntity> ingredients = ingredientJpaRepository
                                        .findAllById(allMissingIngredientIds);
                ...
                List<RecipeRecommendationDto> recommendations = candidateRecipes.stream()
                                .map(recipe -> calculateRecommendation(recipe, userIngredientIds,
                                                finalIngredientNameMap))
                                .sorted((a, b) -> Integer.compare(b.getPriorityScore(), a.getPriorityScore())) // 우선순위
                                                                                                               // 높은 순
                                .limit(10) // 최대 10개
        ...
        private RecipeRecommendationDto calculateRecommendation(RecipeEntity recipe, Set<Long> userIngredientIds,
                ...
                List<MissingIngredientDto> missingRequired = requiredIds.stream()
                                .filter(id -> !userIngredientIds.contains(id))
                                .sorted()
                                .map(id -> new MissingIngredientDto(id,
                                                ingredientNameMap.getOrDefault(id, "알 수 없는 재료")))
                                .collect(Collectors.toList());
                ...
                int requiredMatchRate = requiredIds.isEmpty() ? 100
                                : (int) ((double) (requiredIds.size() - missingRequired.size()) / requiredIds.size()
                                                * 100);

                int optionalMatchRate = optionalIds.isEmpty() ? 100
                                : (int) ((double) (optionalIds.size() - missingOptional.size()) / optionalIds.size()
                                                * 100);
                ...
                // 기본 locale(ko-KR)의 description 조회
                String description = recipeI18nJpaRepository
                                .findByRecipeIdAndLocale(recipe.getRecipeId(), "ko-KR")
                                .map(RecipeI18nEntity::getDescription)
                                .orElse(null);
